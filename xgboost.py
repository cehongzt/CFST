...
x_trains,x_tests,y_trains,y_tests=train_test_split(dems,objects,test_size=0.2,shuffle=True)
x_trainm,x_testm,y_trainm,y_testm=train_test_split(demm,objectm,test_size=0.2,shuffle=True)
x_trainl,x_testl,y_trainl,y_testl=train_test_split(deml,objectl,test_size=0.2,shuffle=True)
print(y_testl)
import joblib
...
for train_indices,test_indices in kf.split(dems):
    ...
    xgbtRLRs=xgbt.XGBRegressor(colsample_bytree=0.6,subsample=0.5,max_depth=3,n_estimators=100,max_delta_step=0,learning_rate=0.32,reg_alpha=0.5,min_child_weight= 1.99, reg_lambda=0.229)
...
for train_indicem, test_indicem in kf.split(demm):
    ...
    xgbtRLRm = xgbt.XGBRegressor(colsample_bytree=0.6, subsample=0.5, max_depth=3, n_estimators=100, max_delta_step=0,
                                 learning_rate=0.32, reg_alpha=0.5, min_child_weight=1.99, reg_lambda=0.229)
    xgbtRLRm.fit(X_trainM, Y_trainM)

    y_predM = xgbtRLRm.predict(X_testM)
    RSMm = np.sqrt(mean_squared_error(Y_testM, (y_predM)))

    RMSE2.append(RSMm)

    if RSMm <= min(RMSE2) and len(RMSE2) != 0:
        joblib.dump(filename='xgboost_m.model1', value=xgbtRLRm)
print(np.mean(RMSE2))
RMSE3=[]
...
for train_indicel, test_indicel in kf.split(deml):
    ...
    xgbtRLRl = xgbt.XGBRegressor(colsample_bytree=0.6, subsample=0.5, max_depth=3, n_estimators=100, max_delta_step=0,
                                 learning_rate=0.32, reg_alpha=0.5, min_child_weight=1.99, reg_lambda=0.229)
    xgbtRLRl.fit(X_trainl, Y_trainl)

    y_predl = xgbtRLRl.predict(X_testl)
    RSMl = np.sqrt(mean_squared_error(Y_testl, (y_predl)))

    RMSE3.append(RSMl)

    if RSMl <= min(RMSE3) and len(RMSE3) != 0:
        joblib.dump(filename='xgboost_l.model1', value=xgbtRLRl)
print(np.mean(RMSE3))
